...
#' @param stitch_input_file Character. RDS base name (no path, no extension). Standard pipeline:
#'   \code{"ECCC_Climate_Data_Stitched"}.
#' @param orig_input_file Character. RDS base name (no path, no extension). Standard pipeline:
#'   \code{"ECCC_Climate_Data_Updated"}.
#' @param output_file Character. RDS base name (no path, no extension). Standard pipeline:
#'   \code{"ECCC_Climate_Data_Merged"}.
#' @param extension Character. File extension including dot. Default \code{".rds"}.
#' @return A dataframe of a merged dataset and writes RDS to disk
#' @export
...

{
  ...
  stitch_path <- file.path(filepath, paste0(stitch_input_file, extension))
  orig_path   <- file.path(filepath, paste0(orig_input_file, extension))
  output_path <- file.path(filepath, paste0(output_file, extension))
  if (!file.exists(stitch_path)) stop("Input not found: ", stitch_path)
  if (!file.exists(orig_path))   stop("Input not found: ", orig_path)

  data.stitch <- readRDS(stitch_path)
    ...
    if(length(id) != 1) {
      ...
      for(j in head(seq_along(id),-1)) {
        ...
        for(m in variables) {
          ...
          # get common dates between df merge and df2
          common_dates <- intersect(df.merge$date, df.2$date)

          if(length(common_dates) > 0) {
            # Filter both dataframes to only include common dates
            df.merge.subset <- df.merge[df.merge$date %in% common_dates, ]
            df.2.subset <- df.2[df.2$date %in% common_dates, ]

            # Ensure they're in the same order by date
            df.merge.subset <- df.merge.subset[order(df.merge.subset$date), ]
            df.2.subset <- df.2.subset[order(df.2.subset$date), ]

            # Double-check that they have the same number of rows
            if(nrow(df.merge.subset) == nrow(df.2.subset)) {

              df.merge.2.1 <- data.frame(
                "date" = common_dates,
                "parameter" = paste(names[m]),
                "logical" = is.na(df.merge.subset[[m]]) & !is.na(df.2.subset[[m]]),
                stringsAsFactors = FALSE)

              df.merge.2 <- dplyr::bind_rows(df.merge.2, df.merge.2.1)

            } else {
              # If they still don't match, skip this variable
              warning(paste("Skipping variable", names[m], "- row count mismatch after filtering"))
            }
          }
        }

        # if df.merge.2 is empty, create minimal df.merge.2 df to prevent downstream errors
        if(nrow(df.merge.2) == 0) {
          cat("df.merge.2 is empty - creating fallback dataframe\n")
          # new df
          df.merge.2 <- data.frame(
